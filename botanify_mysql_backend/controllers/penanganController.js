import { getPenangananById, getPenangananPenyakitByNama, getPenangananHamaByNama, addPenanganan, updatePenanganan, deletePenanganan } from '../models/penangananModel.js';
...
const getPenangananPenyakitByNamaHandler = async (req, res) => {
  const { nama_penyakit } = req.params;
  try {
    const result = await getPenangananPenyakitByNama(nama_penyakit);
    if (result.length === 0) {
      return res.status(404).send('Penanganan tidak ditemukan');
    }
    res.json(result);
...
const getPenangananHamaByNamaHandler = async (req, res) => {
  const { nama_hama } = req.params;
  try {
    const result = await getPenangananHamaByNama(nama_hama);
    if (result.length === 0) {
      return res.status(404).send('Penanganan tidak ditemukan');
    }
    res.json(result);
...
export { getPenangananByIdHandler, getPenangananPenyakitByNamaHandler, getPenangananHamaByNamaHandler, addPenangananHandler, updatePenangananHandler, deletePenangananHandler };
